...
vector< vector<Point2f> > allObjCorners2;
 ...
 * @return none
 */
void setChessboardCorners(Size boardSize, float squareSize, vector<Point3f>& objCorners, vector<Point2f>& objCorners2)
{
    for( int i = 0; i < boardSize.height; i++ )
        for( int j = 0; j < boardSize.width; j++ ){
            objCorners.push_back(Point3f(float(j*squareSize),float(i*squareSize), 0));
            objCorners2.push_back(Point2f(float(j*squareSize),float(i*squareSize)));
        }
                                ...
                                Mat& distCoeffs,vector<Mat>& rvecs, vector<Mat>& tvecs)
{
    ...
    for(vector<string>::iterator it=sampleFiles.begin();it!=sampleFiles.end();it++){
        ...
        vector<Point2f> objCorners2;
        setChessboardCorners(boardSize, squareSize, objCorners, objCorners2);
        ...
        allObjCorners2.push_back(objCorners2);
    }

    
    Mat homo=findHomography(allImgCorners[0],allObjCorners2[0], CV_RANSAC);
    cout<<"homo1:\n\t"<<homo<<endl;

    vector<Point3f> homogeneous_pt;
    convertPointsToHomogeneous(allImgCorners[0],homogeneous_pt);

    Mat coord;

    Mat(homogeneous_pt[1]).convertTo(coord,CV_64FC1);
...
int main()
{
    ...
    for(int i=SNAP_MIN+7; i<=SNAP_MAX; i++){
